...
template <typename T>
std::unique_ptr<T> Allocator::make_unique(std::size_t count) noexcept {
    if (count == 0) {
        return std::unique_ptr<T>(nullptr);
    }
    try {
...
std::unique_ptr<void, decltype(&std::free)> Allocator::aligned_alloc(
    ...
    if (alignment == 0 || size == 0) {
        return std::unique_ptr<void, decltype(&std::free)>();
    }

    // Ensure alignment is a power of two
    if (!std::is_power_of_two(alignment)) {
        return std::unique_ptr<void, decltype(&std::free)>();
    }
...
template std::unique_ptr<int[]> Allocator::make_unique_array<int>(std::size_t) noexcept;
template std::unique_ptr<char[]> Allocator::make_unique_array<char>(std::size_t) noexcept;
template std::unique_ptr<long[]> Allocator::make_unique_array<long>(std::size_t) noexcept;

template std::unique_ptr<int> Allocator::make_unique<int>(std::size_t) noexcept;
template std::unique_ptr<char> Allocator::make_unique<char>(std::size_t) noexcept;
template std::unique_ptr<long> Allocator::make_unique<long>(std::size_t) noexcept;
